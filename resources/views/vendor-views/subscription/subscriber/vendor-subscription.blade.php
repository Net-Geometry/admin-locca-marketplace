                    ...
                    <div class="check--grid-wrapper mt-3 max-w-850px">
                        ...
                        <div>
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <span class="form-check-label text-dark">{{ $store?->module->module_type == 'rental' && rental_module_published_status('Rental') ? translate('messages.unlimited_trips') : translate('messages.unlimited_orders') }}</span>
                                @else
                                <span class="form-check-label text-dark"> {{ $store?->store_sub_update_application?->package?->max_order }} {{
                                   $store?->module->module_type == 'rental' && rental_module_published_status('Rental') ? translate('messages.Trips') : translate('messages.Orders') }} <small>({{ $store?->store_sub_update_application?->max_order }} {{ translate('left') }}) </small> </span>
                                @endif
                            </div>
                        </div>
                        ...
                        <div>
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <span class="form-check-label text-dark">{{ $store?->module->module_type == 'rental' && rental_module_published_status('Rental') ? translate('messages.unlimited_Upload') : translate('messages.unlimited_item_Upload')
                                    }}</span>
                                @else
                                <span class="form-check-label text-dark"> {{ $store?->store_sub_update_application?->max_product }} {{
                                   $store?->module->module_type == 'rental' && rental_module_published_status('Rental') ? translate('messages.Upload') : translate('messages.product_Upload') }} <small>({{ $store?->store_sub_update_application?->max_product  - $store->items_count > 0 ? $store?->store_sub_update_application?->max_product  - $store->items_count : 0 }} {{ translate('left') }}) </small></span>
                                @endif
                            </div>
                        </div>
                            ...
                            <div class="__plan-item hover {{ $store?->store_sub_update_application?->package_id == $package->id  && $store->store_business_model != 'commission'  ? 'active' : ''}}">
                                <div class="inner-div">
                                    ...
                                    <ul class="info">
                                        ...
                                        <li>
                                            <i class="tio-checkmark-circle"></i> <span>  {{$store?->module->module_type == 'rental' && rental_module_published_status('Rental') ? translate('messages.unlimited_trips') :  translate('messages.Unlimited_Orders') }} </span>
                                        </li>
                                        @else
                                        <li>
                                            <i class="tio-checkmark-circle"></i> <span>  {{ $package->max_order }} {{$store?->module->module_type == 'rental' && rental_module_published_status('Rental') ? translate('messages.trips') : translate('messages.Orders') }} </span>
                                        </li>
                                        @endif
