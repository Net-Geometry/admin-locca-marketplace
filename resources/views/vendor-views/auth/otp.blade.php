        ...
        <div class="auth-wrapper-right">
            ...
            <div class="otp-card">
                <div class="text-center">
                    ...
                    <div class="mb-2">
                        {{ translate('a_4_digit_verification_code_has_been') }} <br> {{ translate('sent_to') }} <strong>{{ substr($vendor?->phone, 0, 3) . str_repeat('X', strlen($vendor?->phone) - 5) . substr($vendor?->phone, -2) }}</strong>
                    </div>
                    <div>{{ translate('enter_the_verification_code') }}</div>
                    <div class="mt-4">
                        <form action="{{ route('vendor.verify-phone-otp') }}" method="POST" class="otp-form">
                            @csrf
                            <input type="hidden" name="reset_token" value="342">
    ...
    $('.otp_resend').on('click', function () {
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.errors == 'vendor_not_found') {
                    toastr.error('{{ translate('vendor_not_found') }}', {
                        CloseButton: true,
                        ProgressBar: true
                    });
                }
                if (data.otp_fail == 'phone_verified') {
                    toastr.error('{{ translate('phone_verified') }}', {
                        CloseButton: true,
                        ProgressBar: true
                    });
                }
                if (data.success == 'otp_fail') {
                    toastr.error('{{ translate('otp_fail') }}', {
                        CloseButton: true,
                        ProgressBar: true
                    });
                }
