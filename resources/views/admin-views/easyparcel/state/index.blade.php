                            ...
                            @if ($language)
                                <div class="form-group lang_form" id="default-form">
                                    ...
                                    <input type="text" name="name[]" value="{{ old('name.0') }}" class="form-control"
                                        placeholder="{{ translate('messages.new_state') }}" maxlength="191">
                                </div>
                                ...
                                @foreach ($language as $key => $lang)
                                    <div class="form-group d-none lang_form" id="{{ $lang }}-form">
                                        ...
                                        <input type="text" name="name[]" value="{{ old('name.' . $key + 1) }}"
                                            class="form-control" placeholder="{{ translate('messages.new_state') }}"
                                            maxlength="191">
                                    </div>
                            ...
                            @else
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name" class="form-control"
                                        placeholder="{{ translate('messages.new_state') }}" value="{{ old('name') }}"
                                        maxlength="191">
                                </div>
                                <input type="hidden" name="lang[]" value="default">
                            @endif
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                                  ...
                                  <select name="state_code" id="stateCodeSelect" class="form-control js-select2-custom">
                                      ...
                                      @foreach(EASY_PARCEL_STATE_CODE as $state)
                                          <option value="{{ $state['code'] }}" 
                                                  @if(old('state_code') == $state['code']) selected @endif>
        ...
        <div class="card mt-3">
            <div class="card-header py-2 border-0">
                <div class="search--button-wrapper">
                    <h5 class="card-title">{{ translate('messages.state_list') }}<span
                            class="badge badge-soft-dark ml-2" id="itemCount">{{ $easyPercelStates->total() }}</span></h5>
                    ...
                    <form class="search-form">
                        ...
                        <div class="input-group input--group">
                            <input type="search" name="search" value="{{ request()?->search ?? null }}"
                                ...
                                placeholder="{{ translate('messages.search_states') }}"
                                aria-label="{{ translate('messages.ex_:_states') }}">
            ...
            <div class="card-body p-0">
                <div class="table-responsive datatable-custom">
                    <table id="columnSearchDatatable"
                        ...
                        <tbody id="table-div">
                            @foreach ($easyPercelStates as $key => $state)
                                <tr>
                                    <td>{{ $key + $easyPercelStates->firstItem() }}</td>
                                    <td>{{ $state->id }}</td>
                                    <td>
                                      ...
                                      <td>
                                        <label class="toggle-switch toggle-switch-sm"
                                            for="stocksCheckbox{{ $state->id }}">
                                            <input type="checkbox"
                                                data-url="{{ route('admin.business-settings.easy-parcel.state.status', [$state['id'], $state->status ? 0 : 1]) }}"
                                                class="toggle-switch-input redirect-url"
                                                id="stocksCheckbox{{ $state->id }}"
                                    ...
                                    <td>
                                        <div class="btn--container justify-content-center">
                                            <a class="btn action-btn btn--primary btn-outline-primary"
                                                href="{{ route('admin.business-settings.easy-parcel.state.edit', [$state['id']]) }}"
                                                title="{{ translate('messages.edit_category') }}"><i
                                                    class="tio-edit"></i>
                                            </a>
                                            <a class="btn action-btn btn--danger btn-outline-danger form-alert"
                                                href="javascript:" data-id="category-{{ $state['id'] }}"
                                                data-message="{{ translate('Want to delete this state') }}"
                                                title="{{ translate('messages.delete_state') }}"><i
                                                    class="tio-delete-outlined"></i>
                                            </a>
                                            <form action="{{ route('admin.business-settings.easy-parcel.state.delete', [$state['id']]) }}"
                                                method="post" id="category-{{ $state['id'] }}">
                                                @csrf @method('delete')
                                            </form>
                                        </div>
                                    </td>
            ...
            @if (count($easyPercelStates) !== 0)
                <hr>
            @endif
            <div class="page-area">
                {!! $easyPercelStates->appends($_GET)->links() !!}
            </div>
            @if (count($easyPercelStates) === 0)
