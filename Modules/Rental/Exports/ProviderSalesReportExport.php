...
class ProviderSalesReportExport implements  FromView, ShouldAutoSize, WithStyles,WithColumnWidths ,WithHeadings, WithEvents
{
    ...
    {
        return view('rental::file-exports.provider-sales-report', [
            'data' => $this->data,
        ]);
    }
    ...
    public function styles(Worksheet $sheet) {
        ...
        return [
            ...
            'A1:F'.$this->data['vehicles']->count() +4 => [
                'borders' => [
                    'allBorders' => [
    ...
    public function setImage($workSheet) {
        $this->data['vehicles']->each(function($item,$index) use($workSheet) {
            $drawing = new Drawing();
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                $event->sheet->getStyle('A4:F'.$this->data['vehicles']->count() +4)
                    ->getAlignment()
                    ->setHorizontal(Alignment::HORIZONTAL_CENTER)
                    ->setVertical(Alignment::VERTICAL_CENTER);
