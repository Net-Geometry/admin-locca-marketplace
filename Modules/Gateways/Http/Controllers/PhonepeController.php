...
class PhonepeController extends Controller
{
    ...
    {
        $config = $this->config_values;
        if ($request->code == "PAYMENT_SUCCESS" && $request->transactionId && $payment_data = $this->payment::where(['transaction_id' => $request->transactionId])->first()) {
            $response = Http::withHeaders([
        ...
        $payment_data = $this->payment::where(['transaction_id' => $request->transactionId])->first();
        if (isset($payment_data) && function_exists($payment_data->failure_hook)) {
            call_user_func($payment_data->failure_hook, $payment_data);
        }
        return $this->payment_response($payment_data, 'fail');
