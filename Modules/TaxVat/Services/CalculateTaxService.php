...
namespace Modules\TaxVat\Services;

use Modules\TaxVat\Entities\OrderVatTaxData;
...
class CalculateTaxService
{
    ...
    public function getCalculatedTax($data = [])
    {
        ...
        $systemTaxVat = SystemTaxVat::with('additionalData')
        ...
        if ($systemTaxVat?->is_incuded) {
         ...
         if ($systemTaxVat?->tax_type == 'product_wise') {
            ...
            foreach ($data['productIds'] as $productId) {
                $taxVatIds = TaxOnMultiData::where('data_type', $data['modelClass'])->where('data_id', $productId)->where('system_tax_vat_id', $systemTaxVat->id)->pluck('tax_vat_id')->toArray();
        ...
        elseif ($systemTaxVat?->tax_type == 'category_wise') {
            ...
            foreach ($data['categoryIds'] as $categoryId) {
                $taxVatIds = TaxOnMultiData::where('data_type', $data['modelClass'])->where('data_id', $categoryId)->where('system_tax_vat_id', $systemTaxVat->id)->pluck('tax_vat_id')->toArray();
                $productWiseTaxData =  $this->calculateTax(
                    systemTaxVat: $systemTaxVat,
                    amount: $data['amount'],
                    taxIds: $taxVatIds,
          ...
          return $this->calculateTax(
                ...
                taxIds: $systemTaxVat->tax_vat_ids,
                taxPayer: $data['taxPayer'],
                storeData: $data['storeData'],
                orderId: $data['orderId'],
                countryCode: $data['countryCode']
    ...
    {
        $taxRatePercent = TaxVat::whereIn('id', $taxIds)->select('id', 'name', 'tax_rate')->get();
        $totalTaxPercent = 0;
        $totalTaxamount = 0;
        ...
        foreach ($taxRatePercent as $taxRate) {
            ...
            if ($storeData) {
                $orderTaxData = new OrderVatTaxData();
                ...
                $orderTaxData->tax_vat_id = $taxRate->id;
                $orderTaxData->system_tax_vat_id = $systemTaxVat->id;
